...
```
http://127.0.0.1:9880/tts?text=我是一个粉刷匠，粉刷本领强。我要把那新房子，刷得更漂亮。刷了房顶又刷墙，刷子像飞一样。
哎呀我的小鼻子，变呀变了样。&text_lang=zh&ref_audio_path=mengpai.wav&prompt_lang=zh&prompt_text=呜哇好生气啊！
不要把我跟一斗相提并论！&text_split_method=cut5&batch_size=1&media_type=wav&streaming_mode=true
...
from tools.read_yaml import read_config,read_yaml
...
async def tts_ava_handle(req:dict):
    """
    ...
    check_res = check_ava_params(req)
...
@APP.get("/tts_ava2")
async def tts_ava2(ava: str = None,text: str = None):
    config_value = read_config(ava)
    return config_value
    

@APP.get("/tts_ava")
async def tts_ava(ava: str = None,
                    text: str = None,
                    text_lang: str = 'zh',
                    prompt_lang: str = 'zh',
                    top_k:int = 5,
                    ...
                    streaming_mode:bool = False):
    if ava is None:
        return JSONResponse(status_code=400, content={"message": "ava is required"})
    
    config_value = read_config(ava)
    ref_audio_path = config_value['ref_audio_path']
    prompt_text = config_value['prompt_text']
    # batch_size=1
    req = {
        "text": text,
    ...
    return await tts_ava_handle(req)
    # return config_value
...
@APP.get("/set_ava")
async def set_ava(ava: str = None):
    try:
        if ava in ["", None]:
            return JSONResponse(status_code=400, content={"message": "ava  is required"})
        config_value = read_config(ava)
        t2s_weights_path = config_value['t2s_weights_path']
        vits_weights_path = config_value['vits_weights_path']
        tts_pipeline.init_t2s_weights(t2s_weights_path)
        tts_pipeline.init_vits_weights(vits_weights_path)
        # set_gpt_weights(t2s_weights_path)
        # set_sovits_weights(vits_weights_path)
    except Exception as e:
        return JSONResponse(status_code=400, content={"message": f"set_ava failed", "Exception": str(e)})
        # return config_value
    return JSONResponse(status_code=200, content={"message": "success"})
@APP.get("/test")
async def control(ava: str = None):
    config_value = read_config(ava)
    
    # config_value = read_yaml(ava)
